...
namespace QLVB.Core.Implementation
{
    public class AccountManager : IAccountManager
    {
        ...
        /// <summary>
        /// kiểm tra tên và mật khẩu đăng nhập
        /// </summary>
        ...
        public int ValidateUser(string strusername, string strpassword)
        {
            ...
            if (CheckUserNameLogin(strusername))
            {
                return 0;
            }
            try
            {
                bool isLDAP = AppSettings.IsLDAP;

                if (isLDAP)
                {   // ket noi LDAP

                    int checkLDAP = QLVB.Common.LDAP.LDAPServices.ValidateUserLDAP(strusername, strpassword);
                    switch (checkLDAP)
                    {
                        case -1:    // loi ldap
                            return -1;

                        case 0:     // sai username, pass
                            return 0;
        ...
        /// <summary>
        /// kiem tra danh sach user khong duoc quyen login
        /// </summary>
        /// <returns>
        /// True: khong duoc login
        /// False: duoc login
        /// </returns>
        private bool CheckUserNameLogin(string userlogin)
        {
            string listUserDeny = AppSettings.UserDeny;
            string[] users = listUserDeny.Split(',');
            foreach (string user in users)
            {
                if (userlogin.Equals(user, StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
