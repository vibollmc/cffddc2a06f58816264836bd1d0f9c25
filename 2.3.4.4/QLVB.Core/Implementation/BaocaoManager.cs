...
namespace QLVB.Core.Implementation
{
    public class BaocaoManager : IBaocaoManager
    {
        ...
        public List<Vanbandi> GetSovanbandi(SovanbandiViewModel model)
        {
            try
            {
                ...
                var vanban = _vbdiRepo.Vanbandis
                        .Where(p => model.listidso.Contains((int)p.intidsovanban))
                    //.Where(p => p.intidsovanban == model.idsovanban)
                        .Where(p => p.strngayky >= dtngaykybd)
                        .Where(p => p.strngayky <= dtngaykykt)
                        .OrderBy(p => p.strngayky)
                        .ThenBy(p => p.intso)
                        .ToList();
                return vanban;
            }
            ...
            try
            {
                ...
                var vanban = _vbdenRepo.Vanbandens
                    //.Where(p => p.intidphanloaivanbanden == model.idloaivanban)
                        .Where(p => model.listidloai.Contains((int)p.intidphanloaivanbanden))
                        .Where(p => p.strngayden >= dtngaydenbd)
                        .Where(p => p.strngayden <= dtngaydenkt)
                        .OrderBy(p => p.strngayden)
                        .ThenBy(p => p.intsoden)
        ...
        public string GetTenLoaivanban(string listidloai)
        {
            try
            {
                List<int> listid = new List<int>();
                ...
                var loaivb = _plvanbanRepo.GetAllPhanloaiVanbans.Where(p => listid.Contains(p.intid));
                string strtenloai = string.Empty;
                foreach (var p in loaivb)
                {
                    strtenloai += p.strtenvanban + ", ";
                }
                return strtenloai;
            }
            catch
            ...
            try
            {
                ...
                var vanban = _vbdiRepo.Vanbandis
                    //.Where(p => p.intidphanloaivanbandi == model.idloaivanban)
                        .Where(p => model.listidloai.Contains((int)p.intidphanloaivanbandi))
                        .Where(p => p.strngayky >= dtngaykybd)
                        .Where(p => p.strngayky <= dtngaykykt)
                        .OrderBy(p => p.strngayky)
                        .ThenBy(p => p.intso)
